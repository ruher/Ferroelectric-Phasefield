...
E_interval=40;
[E_hyloop,Ez_coor]=generateElectricfield(6e7,E_interval);
%%----------iteration setup----------%%
Px_mean=zeros(length(E_hyloop),1);
Px_history=zeros(iter_num,1);
[x,y]=meshgrid(-scale/2:scale/2-1,-scale/2:scale/2-1);

j=1;
iter_num=zeros(length(E_hyloop),1);
for i=1:length(E_hyloop)
    if i<=15
        iter_num(i)=500;
    else
        iter_num(i)=300;
...
rhox=interfield_history{15,1};
rhoy=interfield_history{15,2};
...
domain_history=cell(length(E_hyloop),2);
for E1=E_hyloop
    Ex=E1*ones(scale);
    ...
    for i=1:iter_num(j)
        ...
        dG_GL_k_Px2=d*fft2(ifftshift(dG_GL_k_Px2_alpha1+dG_GL_k_Px2_alpha11+dG_GL_k_Px2_alpha12+dG_GL_k_Px2_alpha111+dG_GL_k_Px2_alpha112+dG_GL_k_Px2_alpha1111+dG_GL_k_Px2_alpha1112+dG_GL_k_Px2_alpha1122-Ex-rhox));
        dG_GL_k_Py2=d*fft2(ifftshift(dG_GL_k_Py2_alpha1+dG_GL_k_Py2_alpha11+dG_GL_k_Py2_alpha12+dG_GL_k_Py2_alpha111+dG_GL_k_Py2_alpha112+dG_GL_k_Py2_alpha1111+dG_GL_k_Py2_alpha1112+dG_GL_k_Py2_alpha1122-rhoy));
...
plot(E_hyloop,Px_mean);
figure;
quiver(x,y,domain_history{80,1},domain_history{80,2});