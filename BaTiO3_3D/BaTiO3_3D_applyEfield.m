...
E_interval=100;
[E_hyloop,Ez_coor]=generateElectricfield(8e7,E_interval);
%%----------iteration setup----------%%
relaxtime=300;
iter_num=length(E_hyloop)*relaxtime;
Pz_mean=zeros(length(E_hyloop),1);
Px_history=zeros(iter_num,1);
[x,y,z]=meshgrid(-scalex/2:scalex/2-1,-scaley/2:scaley/2-1,-scalez/2:scalez/2-1);
for i=1:iter_num
    E1=E_hyloop(ceil(i/relaxtime));
    Ez=E1*ones(scalex,scaley,scalez);
    ...
    dG_GL_k_Pz2=d*fftn(ifftshift(dG_GL_k_Pz2_alpha1+dG_GL_k_Pz2_alpha11+dG_GL_k_Pz2_alpha12+dG_GL_k_Pz2_alpha111+dG_GL_k_Pz2_alpha112+dG_GL_k_Pz2_alpha123+dG_GL_k_Pz2_alpha1111+dG_GL_k_Pz2_alpha1112+dG_GL_k_Pz2_alpha1122+dG_GL_k_Pz2_alpha1123-Ez));
    ...
    if rem(i,relaxtime)==0
      Pz_mean(i/relaxtime)=mean2(Pz2);
    end

end
plot(E_hyloop,Pz_mean);
figure;
